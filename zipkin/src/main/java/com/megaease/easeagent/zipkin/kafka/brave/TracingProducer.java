...
public final class TracingProducer<K, V> implements Producer<K, V> {
    ...
    public final CurrentTraceContext currentTraceContext;
    final Tracer tracer;
    ...
    public void afterSend(Tracer.SpanInScope ws, Span span, Throwable throwable) {
        if (throwable != null) span.error(throwable).finish();
        ws.close();
    }
