...
public class PluginRegistryTest {
    ...
    public void isVersion() {
        ...
        assertTrue(PluginRegistry.isVersion(points, plugin, new Configs(Collections.emptyMap())));
        Configs configs = new Configs(Collections.singletonMap("plugin.test_domain.test_namespace.version", "spring_boot_2_x"));
        assertFalse(PluginRegistry.isVersion(points, plugin, configs));
        points.versions = new HashSet<>(Arrays.asList(Points.DEFAULT_VERSION, "spring_boot_2_x"));
        assertTrue(PluginRegistry.isVersion(points, plugin, configs));
        ...
        assertFalse(PluginRegistry.isVersion(points, plugin, configs));
        configs = new Configs(Collections.singletonMap("plugin.test_domain.test_namespace.version", "spring_boot_3_x"));
        points.versions = new HashSet<>(Arrays.asList(Points.DEFAULT_VERSION, "spring_boot_2_x"));
        assertFalse(PluginRegistry.isVersion(points, plugin, configs));
    }
