...
import com.megaease.easeagent.plugin.dubbo.interceptor.trace.apache.DubboAsyncCallback;
...
public class ApacheDubboCtxUtils {
	private static final String CLIENT_REQUEST_CONTEXT = ApacheDubboCtxUtils.class + ".CLIENT_REQUEST_CONTEXT";
	private static final String SERVICE_REQUEST_CONTEXT = ApacheDubboCtxUtils.class + ".SERVICE_REQUEST_CONTEXT";
	...
	public static void finishSpan(URL url, DubboTraceConfig config, Context context, Result result, Throwable throwable) {
		boolean isConsumer = url.getParameter(SIDE_KEY).equals(CONSUMER_SIDE);
		if (result instanceof AsyncRpcResult) {
			result.whenCompleteWithContext(new DubboAsyncCallback(isConsumer, config, context, throwable));
		} else {
			doFinishSpan(isConsumer, config, context, result, throwable);
		}
	}

	public static void doFinishSpan(boolean isConsumer, DubboTraceConfig dubboTraceConfig, Context context, Result result, Throwable throwable) {
